angular.module('MetronicApp').controller('AssignPostLostItemController', function ($rootScope, $scope, settings, $translate, $translatePartialLoader, $location,
    AdjusterListService, AdjusterPropertyClaimDetailsService, $uibModal, $q, $filter) {
    ...
    $scope.QuotedValue = 0.00;
    $scope.ContactInsured=false;
    ...
    function init() {
        ...
        $scope.CommanObj = {
            ...
            TaxRate: sessionStorage.getItem("AdjusterTaxRate"),
        ...
        var getpromise = AdjusterPropertyClaimDetailsService.getVendorForAssignItems(param);
        ...
        var getVendorService = AdjusterPropertyClaimDetailsService.getVendorServiceList();
    ...
    function GetLostDamagedItem() {
        ...
        var getpromise = AdjusterPropertyClaimDetailsService.getPostLostItemsWithComparables(param);
        getpromise.then(function (success) {
    ...
    $scope.AssignPostLostItem = function () {
        if ($scope.SelectedPostLostItems.length === 0 || $scope.SelectedPostLostItems === null || $scope.SelectedParticipant.length == 0) {
            ...
            var param = {
                ...
                "canContactInsured": $scope.ContactInsured
            };
             
            //var assignPostLostItem = AdjusterPropertyClaimDetailsService.SaveServiceRequested(param);
                            ...
                            if (category == VendorServices.category.name) {
                                angular.forEach(VendorServices.contentServices, function (contentService) {
                                    if (contentService.service == service.name) {
                                        ...
                                        specialist.providedService.push({ "Id": contentService.id, "Name": contentService.service, "serviceCharge": contentService.serviceCharge })
    ...
    function getMinimumCost() {
        $scope.QuotedValue = 0.00;
        angular.forEach($scope.PricingSpecialist, function (Specialist) {
         
            if($scope.SelectedParticipant.vendorId==Specialist.vendorId)
            {
             
                angular.forEach(Specialist.providedService,function(providedservice)
                {
                    if (providedservice.serviceCharge != null)
                    {
                        $scope.QuotedValue += providedservice.serviceCharge;
                    }
                });
    ...
    //$scope.FilterSpecialistByCategory = FilterSpecialistByCategory;
    //function FilterSpecialistByCategory() {
    //    var loopPromises = [];
    //    $scope.Selected = [];
    ...
    //        angular.forEach($scope.PricingSpecialist, function (specialist) {
    //            if (specialist.vendorProvidedServices !== null) {
    //                angular.forEach(specialist.vendorProvidedServices, function (services) {

    //                    if (services.category.name == categories) {
    //                        count++;
    //                        $scope.FilteredCategories.push(services)
    //                    }
    //                });
    //            }
    //            if (count > 0) {
    //                var index = $scope.Selected.indexOf(specialist);
    //                if (index == -1) {

    //                    specialist.vendorProvidedServices = [];
    //                    specialist.vendorProvidedServices.push($scope.FilteredCategories);
    //                    $scope.Selected.push(specialist);
    //                }
    //            }
    //            count = 0;
    //            $scope.FilteredCategories = [];
    ...
    //    });
    //    //For Binding Filtered List
    //    $q.all(loopPromises).then(function () {
    //        
    //        $scope.PricingSpecialist = $scope.Selected;
    //        $scope.OriginalPricingSpecialist = $scope.Selected;
    //        angular.forEach($scope.OriginalPricingSpecialist, function (specialist) {
    //            specialist.providedService = [];
    //        });
    //    });
    //    //End Binding Filtered List
    //};
