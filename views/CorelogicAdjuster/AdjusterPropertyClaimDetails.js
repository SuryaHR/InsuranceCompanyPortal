angular.module('MetronicApp').controller('AdjusterPropertyClaimController', function ($uibModal, $state,
    $scope, $filter,CorelogicConstants, $window, $location,$rootScope,
    ...
    $scope.GoToDetails = function (item) {
        ...
        sessionStorage.setItem("BackPage","CorelogicAdjusterPropertyClaimDetails");
    ...
    function SelectServicetype(Servicetype) {
        ...
        if ($scope.Servicetype == 1) {
            ...
            angular.forEach($scope.ContentServiceList[0].subServices, function (subService){
                ...
                if($scope.CommonObj.ClaimProfile == "Jewelry" && subService.service != 'Quote Only' && subService.service != 'Full Inspection'){
                    $scope.salvageContentService.push(subService);
                }
            })
    ...
    $scope.completeEstimate = completeEstimate;
    function completeEstimate() {
    if($scope.FiletrLostDamageList.length==0)
    {
        toastr.remove();
        window.setTimeout(()=>toastr.warning("There are no items to create the Estimate."),100);
        return 0;
    }
      let nonValuedItems = $scope.FiletrLostDamageList?.filter((item) =>
      [
        "CREATED",
        "ASSIGNED",
        "WORK IN PROGRESS",
      ...
      if (nonValuedItems?.length == 0) {
        bootbox.confirm({
          size: "",
          title: "Complete Estimate",
          message:
            "Are you sure you want to complete pricing and create Estimate for " +
            $scope.CommonObj.ClaimNumber +
            " ?",
          closeButton: false,
          className: "modalcustom",
          buttons: {
          ...
          callback: function (result) {
            ...
            var getpromise =
              AdjusterPropertyClaimDetailsService.completeEstimate(param);
            getpromise.then(
              function (success) {
                toastr.remove();
                toastr.success(
                  success.data.message +
                    " Successfully created an Estimate on Claims Connect Application.",
                  "Confirmation"
                );
                $(".page-spinner-bar").addClass("hide");
              },
              function (error) {
                ...
                toastr.error(
                  errorMessage
                    ? errorMessage
                    : "Failed to create an estimate on Claims Connect Application",
                  "Attention"
                );
              }
            );
        ...
        toastr.remove();
        window.setTimeout(()=>toastr.warning("There are "+nonValuedItems?.length+" items pending to add replacement information."),100);
      }
    } 
                ...
                callback: function (result) {
                    if (result) {
                        ...
                        getCategories.then(function (success) {
                            ...
                            getClaimsStatusContentDetails();
                            var updateItemsLogHistory = LineItemService.updateItemsLogHistory(param);
                        }, function (error) {
                            $(".page-spinner-bar").addClass("hide");
                            $scope.ErrorMessage = error.data.errorMessage;
                            toastr.remove();
