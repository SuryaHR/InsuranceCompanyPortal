angular.module('MetronicApp')
    ...
    .controller('AddlAppraisalDetailsController', function ($http, $rootScope, $scope, settings, $location, $translate, $translatePartialLoader, $window,
        $filter, $timeout, InsuranceAgentHomeService, AuthHeaderService, AppraisalService, InsuranceAgentReviewService, $uibModal, UnderWriterDashboardService) {
        ...
        $scope.defaultPearlItem = [{ "type": "", "shape": "", "quality": "", "size": "", "color": "", "lengthOfStrand": "", "typeOfClasp": "", "quantity": "" }];
        ...
        function init() {
            ...
            GetGlobaldata.then(function (success) {
                let ConfigSettings = success.data.data;
        ...
        function getAppraisalDetails() {
            ...
            details.then(function (success) {
                ...
                if (success.data.data.appraisalDataDTO.status == 'Underwriter Review' || success.data.data.appraisalDataDTO.status == 'CREATED') {
                ...
                if (appraisalDetails.policyholderDetails) {
                    $scope.policyCurrentTerm = $filter('DateFormatMMddyyyy')(appraisalDetails.policyholderDetails.policyEffectiveDate) + " to " + $filter('DateFormatMMddyyyy')(appraisalDetails.policyholderDetails.policyRenewalDate),
                        $scope.PolicyholderDetails.policyNumber = (appraisalDetails.policyholderDetails.policyNumber) ? appraisalDetails.policyholderDetails.policyNumber : "";
                }
                // $scope.premiumValue = ($scope.PolicyholderDetails.address && $scope.PolicyholderDetails.address.state && $scope.PolicyholderDetails.address.state.premiumValueWrtState)?$scope.PolicyholderDetails.address.state.premiumValueWrtState:0;

                // Policyholder details
                if (appraisalDetails.policyholderDetails) {
                    $scope.PolicyholderDetails.primaryPolicyHolderFullName = $filter('titleCase')(appraisalDetails.policyholderDetails.primaryPolicyHolderLname) + ', ' + $filter('titleCase')(appraisalDetails.policyholderDetails.primaryPolicyHolderFname);
                    $scope.PolicyholderDetails.primaryPolicyHolderCellphoneNo = appraisalDetails.policyholderDetails.primaryPolicyHolderCellphoneNo
                    $scope.PolicyholderDetails.primaryPolicyHolderEmailId = appraisalDetails.policyholderDetails.primaryPolicyHolderEmailId;                    
                    $scope.PolicyholderDetails.secPolicyHolderFullName = $filter('titleCase')(appraisalDetails.policyholderDetails.secondaryPolicyHolderLname) + ', ' + $filter('titleCase')(appraisalDetails.policyholderDetails.secondaryPolicyHolderFname);
                    $scope.PolicyholderDetails.secPolicyHolderCellphoneNo = appraisalDetails.policyholderDetails.secondaryPolicyHolderCellphoneNo
                    $scope.PolicyholderDetails.secPolicyHolderEmailId = appraisalDetails.policyholderDetails.secondaryPolicyHolderEmailId;                                                            
                    var broadcastData = {
                        "policyholder": $scope.PolicyholderDetails.primaryPolicyHolderFullName   
                    }
                    $rootScope.$broadcast('eventEmitedPolicyHolderName', broadcastData);
                }
                else {
                    $scope.PolicyholderDetails.primaryPolicyHolderFullName = null;
                    $scope.PolicyholderDetails.primaryPolicyHolderCellphoneNo = null;
                    $scope.PolicyholderDetails.primaryPolicyHolderEmailId = null;
                    $scope.PolicyholderDetails.secPolicyHolderFullName = null;
                    $scope.PolicyholderDetails.secPolicyHolderCellphoneNo = null;
                    $scope.PolicyholderDetails.secPolicyHolderEmailId = null;
                }
                ...
                oldInsurancePremiumCost = roundOf2Decimal(appraisalDetails.oldInsurancePremiumCost);
                var sc_insuranceReplacementValue = roundOf2Decimal(appraisalDetails.sc_insuranceReplacementValue);

                var insuranceAmountRequested = roundOf2Decimal(appraisalDetails.insuranceAmountRequested);
                ...
                $scope.Appraisal = {
                    ...
                    "speedCheckAppraisalDate": appraisalDetails.speedcheckAppraisalDate,
                    "insuranceAmountRequested": insuranceAmountRequested,
                    "sc_insuranceReplacementValue": sc_insuranceReplacementValue
                ...
                */
                ...
                $scope.speedcheckSubmitDate = $scope.Appraisal.speedCheckAppraisalDate;
                ...
                if (appraisalDetails.itemCategory && appraisalDetails.itemCategory.atttibuteValue) {
                    ...
                    if (appraisalDetails.itemCategory.atttibuteValue === 'Wine') {
                        $scope.showMountingandSetting = false;
                        $scope.isPearl = false;
                        $scope.brand = true;
                        $scope.characteristics = true;
                        $scope.location = true;
                        if (appraisalDetails.appraisalWineDTO) {
                            $scope.Appraisal.appraisalWineDTO = appraisalDetails.appraisalWineDTO;

                        }


                    }
                    else if (appraisalDetails.itemCategory.atttibuteValue === 'Arts') {
                        $scope.showMountingandSetting = false;
                        $scope.isPearl = false;
                        $scope.artAndMaterial = true;
                        $scope.dimensionAndArtifact = true;
                        $scope.saleDetails = true;
                        if (appraisalDetails.appraisalArtDTO) {
                            $scope.Appraisal.appraisalArtDTO = appraisalDetails.appraisalArtDTO;
                ...
                angular.forEach(appraisalDetails.attachments, function (ItemFile) {
                    ...
                    $scope.attachmentList.push({ "fileName": words, "url": $scope.serverAddress + name, "type": fileType, "fileData": ItemFile.multipartFiles });
                });
        ...
        function GetAppraisalDropdowns() {
            ...
            promis.then(function (success) {
                ...
                for (var i = 0; i < $scope.AppraisalDropdowns.length; i++) {
                    ...
                    //MEDIA
                    else if (AppraisalDropdown.attributeName == 'MEDIA') {

                        $scope.mediaList = AppraisalDropdown.attributeValue;
                    }
                    else if (AppraisalDropdown.attributeName == 'SPECIFIC_MEDIUM') {

                        $scope.specificMediumList = AppraisalDropdown.attributeValue;
                    }
                    else if (AppraisalDropdown.attributeName == 'VARIETAL') {

                        $scope.wineVarietalList = AppraisalDropdown.attributeValue;
                    }
        ...
        function GeneratDescription(insuranceReplacementCost) {
            ...
            $scope.desc.insuranceReplacementCost = "Insurance replacement value    " + $filter('currency')(parseFloat(insuranceReplacementCost));

            //$scope.Appraisal.newInsurancePremiumCost = insuranceReplacementCost * 0.012;

            var newValue = roundOf2Decimal(insuranceReplacementCost / 1000);
            var newCost = newValue * 12
            $scope.Appraisal.newInsurancePremiumCost = roundOf2Decimal(newCost);
        ...
        function onChangePopulate() {
            ...
            //art and wine
            $scope.artAndMaterial = false;
            $scope.dimensionAndArtifact = false;
            $scope.saleDetails = false;
            $scope.brand = false;
            ...
            if ($scope.metalWeightList)
                elementPos = $scope.metalWeightList.map(function (x) { return x.attributeValueId; }).indexOf(240);
            ...
            //other
            ...
            else if (itemCatagory == 249 && (itemType != null || itemType != "undefined")) {
                ...
                //onChangeTypeGetVarietal(itemType);

            }
            else if (itemCatagory == 250 && (itemType != null || itemType != "undefined")) {
        ...
        function UpdateApprisal(approveStatus) {
            var policyNum = sessionStorage.getItem("policyNumber");
            var createdBy = sessionStorage.getItem("UserId");
            var mountingDetails = {};
            var diamondDetails = $scope.diamondItems;
            ...
            */
            ...
            var details = {
                ...
                "sc_jwelersCost": $scope.totalJeweleryCost,
                "sc_artigemReplacementValue": $scope.totalArtigemReplacementCost,
                "sc_insuranceReplacementValue": $scope.Appraisal.sc_insuranceReplacementValue,
                "sc_retailValue": $scope.totalRetailValue,
                "sc_totalMountingPrice": $scope.Appraisal.ScTotalMountingPrice,
                "sc_totalChainPrice": $scope.Appraisal.ScTotalChainPrice,
                "sc_totalDiamondPrice": $scope.diamondInsuranceReplacementCost,
                ...
                "centreStoneType": ($scope.Appraisal.centeredStone) ? $scope.Appraisal.centeredStone.type : null,
                "chainExists": ($scope.Appraisal.Chain) ? $scope.Appraisal.Chain.isChain : null,
                "appraisalArtDTO": $scope.Appraisal.appraisalArtDTO,
                "appraisalWineDTO": $scope.Appraisal.appraisalWineDTO,
                "insuranceAmountRequested": $scope.Appraisal.insuranceAmountRequested
            };
            //"appraisalDocuments":appraisalDocuments
            $scope.details = details;
            ...
            promis.then(function (success) {
                ...
                if (approveStatus == 'APPROVED' || approveStatus == 'NEED_INFO') {
                    updateRequest(approveStatus);
                }
                else {
                    $timeout(function () {
                        $scope.disableScreen = false;
        ...
        function ClearFilters(param) {
            ...
            $scope.Appraisal.appraisalWineDTO = {};
            $scope.Appraisal.appraisalArtDTO = {};
        ...
        //Save & Update new insurance requests.
        $scope.updateRequest = updateRequest;
        function updateRequest(status) {
            $(".page-spinner-bar").removeClass("hide");
            var param = {
                "rejected": status === 'NEED_INFO' ? true : false,
            ...
            var promis = UnderWriterDashboardService.updateAdditionalAppraisalRequest(param)
            promis.then(function (success) {
                toastr.remove();
                toastr.success("The appraisal was successfully updated.", "Confirmation");
        ...
        //  get country list api/customer/get/country/list
        $scope.getCountryList = getCountryList;
        function getCountryList() {

            var promis = UnderWriterDashboardService.getCountryList();
            promis.then(function (success) {
                //console.log(success.data);
                $scope.countryList = success.data.data;
                ...
                $scope.companyOptions = {
                    ...
                    filter: "contains",
                    // itemTemplate: "<input type='checkbox'/>#:name# ",
                    //headerTemplate:"<input type='checkbox' id='header'><label>All</label>",
        ...
        //onchangeType get varietal dropdown
        $scope.onChangeTypeGetVarietal = onChangeTypeGetVarietal;
        function onChangeTypeGetVarietal(param) {

            //param=33;
            var promis = UnderWriterDashboardService.getWineVarietalList(param);
            promis.then(function (success) {
                //console.log(success.data);
                $scope.wineVarietalList = success.data.appraisalDropdownValue;
            }, function (error) {
                toastr.remove();
                $(".page-spinner-bar").addClass("hide");
                if (error.data.errorMessage != null && angular.isDefined(error.data.errorMessage)) {
            ...
            $(document).ready(function () {
                $("#products").kendoDropDownList({
                    ...
                    select: function (e) {
                        ...
                        if ($scope.Appraisal && $scope.Appraisal.appraisalWineDTO && $scope.location) {
                            $scope.Appraisal.appraisalWineDTO.countryOfSale = {};
                            $scope.Appraisal.appraisalWineDTO.countryOfSale.id = e.dataItem.id;
                            $scope.Appraisal.appraisalWineDTO.countryOfSale.name = e.dataItem.name;
                        }
                        else if ($scope.Appraisal && $scope.Appraisal.appraisalArtDTO && $scope.saleDetails) {
                            $scope.Appraisal.appraisalArtDTO.countryOfSale = {};
                            $scope.Appraisal.appraisalArtDTO.countryOfSale.id = e.dataItem.id;
                            $scope.Appraisal.appraisalArtDTO.countryOfSale.name = e.dataItem.name;
                        }

                        //e.preventDefault();
                        console.log(e);
                    },
                    value: ($scope.location) ? (($scope.Appraisal && $scope.Appraisal.appraisalWineDTO && $scope.Appraisal.appraisalWineDTO.countryOfSale && $scope.Appraisal.appraisalWineDTO.countryOfSale.id) ? $scope.Appraisal.appraisalWineDTO.countryOfSale.id : "") : (($scope.Appraisal && $scope.Appraisal.appraisalArtDTO && $scope.Appraisal.appraisalArtDTO.countryOfSale && $scope.Appraisal.appraisalArtDTO.countryOfSale.id) ? $scope.Appraisal.appraisalArtDTO.countryOfSale.id : ""),
        ...
        $(document).ready(function () {
            $rootScope.$broadcast('updateActiveTab', 'nwInsReq');
        });
