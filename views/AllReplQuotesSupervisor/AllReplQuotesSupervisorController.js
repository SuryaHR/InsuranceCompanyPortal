angular.module('MetronicApp').controller('AllReplQuotesSupervisorController', function ($rootScope, $scope, $location, $translate, $translatePartialLoader, settings, $filter, AuthHeaderService, AllReplQuotesService) {

    //set language
    $translatePartialLoader.addPart('AllReplQuotesSupervisor');
    $translate.refresh();
    ...
    $scope.valueQuotedItems = $scope.FiletrLostDamageList.filter(function (item) {
        ...
        if (item.status && item.status.status.toLowerCase() === 'valued') {
