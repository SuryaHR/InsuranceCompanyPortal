angular.module('MetronicApp')
    ...
    .controller('JewelryCustomComparableController', function ( $rootScope, $scope,  $translate, $translatePartialLoader,
        ...
        $scope.CustomComp = {
            ...
            // default dropdown value
            "leftCenterStone": "Diamond",

            // default dropdown value
            "rightCenterStone": "Diamond",
       ...
       function GetWholeCustomComparable(internal){
            ...
            if($scope.CommonObj.itemReplacementId){
                ...
                wholeJewelryCustomItem.then(function (success) {
                    ...
                    if(success && success.data && success.data.data && success.data.data.itemId!=0){
                        ...
                        $scope.CustomComp.labourChargesNote = success.data.data.labourChargesNote ? success.data.data.labourChargesNote : ""; 

                        $scope.CustomComp.customCharges = success.data.data.customCharges ? success.data.data.customCharges : 0.00; 
                        $scope.CustomComp.customChargesNote = success.data.data.customChargesNote ? success.data.data.customChargesNote : ""; 

                        $scope.CustomComp.materialCost = success.data.data.materialCost ? success.data.data.materialCost : 0.00;
                        $scope.CustomComp.sku = success.data.data.sku ? success.data.data.sku : "";
                        $scope.CustomComp.supplierWebSite = success.data.data.supplierWebSite ? success.data.data.supplierWebSite : ""; 
                        $scope.CustomComp.supplier = success.data.data.supplier ? success.data.data.supplier : ""; 

                        $scope.CustomComp.centerStone = (success.data.data.centerStoneType && success.data.data.centerStoneType!="") ? success.data.data.centerStoneType : $scope.CustomComp.centerStone;
                        $scope.CustomComp.leftCenterStone = (success.data.data.leftStoneType && success.data.data.leftStoneType!="") ? success.data.data.leftStoneType : $scope.CustomComp.leftCenterStone;
                        $scope.CustomComp.rightCenterStone = (success.data.data.rightStoneType && success.data.data.rightStoneType !="") ? success.data.data.rightStoneType : $scope.CustomComp.rightCenterStone;
                        ...
                        $scope.CustomComp.Designer.attributeValueId = success.data.data.designerItem ? parseInt(success.data.data.designerItem) : null;
                        ...
                        // Loose Stone - Gemstone (default value is Gemstone)
                        if($scope.CustomComp.ItemType.attributeValueTypeId == 17){
                            $scope.CustomComp.centerStone = "Gemstone";
                        }

                        // Loose Stone - Pearl (default value is Pearl)
                        if($scope.CustomComp.ItemType.attributeValueTypeId == 27){
                            $scope.CustomComp.centerStone = "Pearl";
                        } 
                        ...
                        $scope.CustomComp.settings = getComponentList(allComparableComponets, 'SETTINGS');
                        $scope.CustomComp.weddingBands = getComponentList(allComparableComponets, 'WEDDING_BAND');

                        $scope.CustomComp.chains = getComponentList(allComparableComponets, 'CHAINS');
                        $scope.CustomComp.leftCenterStoneDiamonds = getComponentList(allComparableComponets, 'LEFT_CENTER_DIAMOND');
                        $scope.CustomComp.leftCenterStoneGemstones = getComponentList(allComparableComponets, 'LEFT_CENTER_GEMSTONE');
                        $scope.CustomComp.rightCenterStoneDiamonds = getComponentList(allComparableComponets, 'RIGHT_CENTER_DIAMOND');
                        $scope.CustomComp.rightCenterStoneGemstones = getComponentList(allComparableComponets, 'RIGHT_CENTER_GEMSTONE');
                        $scope.CustomComp.centerStonePearls = getComponentList(allComparableComponets, 'CENTER_PEARL');

                        $scope.CustomComp.mainWatchs = getComponentList(allComparableComponets, 'MAIN_WATCH');
                        $scope.CustomComp.customAdditions = getComponentList(allComparableComponets, 'CUSTOM_ADDITIONS');
                    ...
                    const itemCategory = $scope.findSelectValById($scope.itemCategoryList, $scope.CustomComp.ItemCategory.attributeValueId, 'attributeValueId');
                    $scope.CustomComp.itemCategoryName = itemCategory[0].atttibuteValue;

                    const itemType = $scope.findSelectValById($scope.itemTypeList, $scope.CustomComp.ItemType.attributeValueTypeId, 'attributeValueTypeId');
                    $scope.CustomComp.itemTypeName = itemType[0].atttibuteType;
        ...
        function markFavouriteJewelryComponent(item) {
            if($scope.ItemDetails.status.status !== $scope.constants.itemStatus.supervisorReview){
                ...
                if(item.itemType == "MAIN_WATCH"){
                    fItem = $scope.CustomComp.mainWatchs.filter(fElm => fElm.itemFavourite == true);
                }
                ...
                if(item.itemType == "CHAINS"){
                    fItem = $scope.CustomComp.chains.filter(fElm => fElm.itemFavourite == true);
                }

                if(item.itemType == "CUSTOM_ADDITIONS"){
                    fItem = $scope.CustomComp.customAdditions.filter(fElm => fElm.itemFavourite == true);
                }

                if(item.itemType == "LEFT_CENTER_DIAMOND"){
                    fItem = $scope.CustomComp.leftCenterStoneDiamonds.filter(fElm => fElm.itemFavourite == true);
                }

                if(item.itemType == "LEFT_CENTER_GEMSTONE"){
                    fItem = $scope.CustomComp.leftCenterStoneGemstones.filter(fElm => fElm.itemFavourite == true);
                }
                if(item.itemType == "RIGHT_CENTER_DIAMOND"){
                    fItem = $scope.CustomComp.rightCenterStoneDiamonds.filter(fElm => fElm.itemFavourite == true);
                }

                if(item.itemType == "RIGHT_CENTER_GEMSTONE"){
                    fItem = $scope.CustomComp.rightCenterStoneGemstones.filter(fElm => fElm.itemFavourite == true);
                }
                // Update selection
        ...
        function sortFavouriteItems(){
            ...
            $scope.CustomComp.chains.sort((a, b) =>  (a.itemFavourite === b.itemFavourite)? 0 : a.itemFavourite? -1 : 1);
            $scope.CustomComp.weddingBands.sort((a, b) =>  (a.itemFavourite === b.itemFavourite)? 0 : a.itemFavourite? -1 : 1);

            $scope.CustomComp.leftCenterStoneDiamonds.sort((a, b) =>  (a.itemFavourite === b.itemFavourite)? 0 : a.itemFavourite? -1 : 1);
            $scope.CustomComp.leftCenterStoneGemstones.sort((a, b) =>  (a.itemFavourite === b.itemFavourite)? 0 : a.itemFavourite? -1 : 1);
            $scope.CustomComp.rightCenterStoneDiamonds.sort((a, b) =>  (a.itemFavourite === b.itemFavourite)? 0 : a.itemFavourite? -1 : 1);
            $scope.CustomComp.rightCenterStoneGemstones.sort((a, b) =>  (a.itemFavourite === b.itemFavourite)? 0 : a.itemFavourite? -1 : 1);  
            
            $scope.CustomComp.mainWatchs.sort((a, b) =>  (a.itemFavourite === b.itemFavourite)? 0 : a.itemFavourite? -1 : 1);
            $scope.CustomComp.customAdditions.sort((a, b) =>  (a.itemFavourite === b.itemFavourite)? 0 : a.itemFavourite? -1 : 1);     
        ...
        function calculateReplacementPrice(internal) {
            ...
            const chainItem = $scope.CustomComp.chains.filter(fElm => fElm.itemFavourite == true);
            const leftDiamondItem = $scope.CustomComp.leftCenterStoneDiamonds.filter(fElm => fElm.itemFavourite == true);
            const leftGemstoneItem = $scope.CustomComp.leftCenterStoneGemstones.filter(fElm => fElm.itemFavourite == true);
            const rightDiamondItem = $scope.CustomComp.rightCenterStoneDiamonds.filter(fElm => fElm.itemFavourite == true);
            const rightGemstoneItem = $scope.CustomComp.rightCenterStoneGemstones.filter(fElm => fElm.itemFavourite == true);

            const mainWatchItem = $scope.CustomComp.mainWatchs.filter(fElm => fElm.itemFavourite == true);
            const customAdditionItem = $scope.CustomComp.customAdditions.filter(fElm => fElm.itemFavourite == true);
            ...
            if(mainWatchItem && mainWatchItem.length!=0){
                $scope.CustomComp.subTotal += mainWatchItem[0].totalReplacementPrice;
            }
            if(customAdditionItem && customAdditionItem.length!=0){
                $scope.CustomComp.subTotal += customAdditionItem[0].totalReplacementPrice;
            }

            if(chainItem && chainItem.length!=0){
                $scope.CustomComp.subTotal += chainItem[0].totalReplacementPrice;
            }

            if(leftDiamondItem && leftDiamondItem.length!=0){
                $scope.CustomComp.subTotal += leftDiamondItem[0].totalReplacementPrice;
            }
            if(leftGemstoneItem && leftGemstoneItem.length!=0){
                $scope.CustomComp.subTotal += leftGemstoneItem[0].totalReplacementPrice;
            }
            if(rightDiamondItem && rightDiamondItem.length!=0){
                $scope.CustomComp.subTotal += rightDiamondItem[0].totalReplacementPrice;
            }
            if(rightGemstoneItem && rightGemstoneItem.length!=0){
                $scope.CustomComp.subTotal += rightGemstoneItem[0].totalReplacementPrice;
            }
            ...
            $scope.CustomComp.totalReplacementPrice = parseFloat($scope.CustomComp.subTotal + $scope.CustomComp.taxesVal+
                                                     ($scope.CustomComp.labourCharges && $scope.CustomComp.labourCharges!="" ? parseFloat($scope.CustomComp.labourCharges) :0)+
                                                     ($scope.CustomComp.customCharges && $scope.CustomComp.customCharges!="" ? parseFloat($scope.CustomComp.customCharges) :0) );
        ...
        $scope.GetDocxDetails = function (item) {
            ...
            $scope.DocxDetails.type = $scope.DocxDetails.FileExtension.substr(1);
        ...
        function SaveComponentsDetails(internal) { 
           ...
           var customItem =  JSON.stringify({
                ...
                "customCharges" : $scope.CustomComp.customCharges,
            ...
            getItemDetailsOnId.then(function (success) {
                ...
                if(internal!=0){
                    ...
                    // if($scope.isReplacement){
                    //     sessionStorage.setItem("JewelryReplSave", "true");
                    // }
        ...
        //findSelectValById(itemCategoryList, CustomComp.ItemCategory.attributeValueId)
        $scope.findSelectValById = findSelectValById;
        function findSelectValById(list, id, idName) {
            if(list!=null)
            var lst = list.filter(fElm => fElm[idName] == id);
            return lst;
        }        
