angular.module('MetronicApp').controller('UserDetailsUploadContoller', function ($translate, $window, $translatePartialLoader, $rootScope, $scope, $location, $interval,
    UserDetailsUploadService, $filter, $timeout) {
    $scope.$on('$viewContentLoaded', function () {
        // initialize core components
        App.initAjax();
    });
    $scope.fileControl;
    $translatePartialLoader.addPart('UserDetailsUpload');
    $translate.refresh();
    $scope.ErrorMessage = "";
    $scope.FileDetails = {
        FileName: null,
    ...
    $scope.TemplateURL = angular.isDefined(sessionStorage.getItem("UserDetailsTemplate")) && sessionStorage.getItem("UserDetailsTemplate") != null ? sessionStorage.getItem("UserDetailsTemplate") : null;
    //trigger fileupload
    ...
    function StartUpload() {
        ...
        var UploadFile = UserDetailsUploadService.UploadVendorDetailsFile(data);
        UploadFile.then(function (success) {
            $scope.uploader.progress = 100;
            $interval.cancel($scope.timer);
            $scope.UserList = success.data.data.userDetails;
    ...
    function Verify() {
        ...
        var paramList = $scope.UserList;
        var UploadItem = UserDetailsUploadService.SaveVendorDetails(paramList);
        UploadItem.then(function (success) {
            ...
            $scope.FialedItem = ((success.data.data.rejected !== null) ? success.data.data.rejected.length : 0);
        }, function (error) {
            StopProgressBarShowFinal();
            HideProgressBar();
