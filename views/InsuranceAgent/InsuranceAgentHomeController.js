angular.module('MetronicApp')
    .controller('InsuranceAgentHomeController', function ($rootScope, $scope, $filter, settings, $http, $location, $translate, $translatePartialLoader, InsuranceAgentHomeService,
        ...
        function init() {
            ...
            //commented getNotifications()
            //  getNotifications();
            //Enabled getAllNotifications()
            $timeout(function () {
                GetPolicyList();
            }, 100);
            getAllNotifications();
        ...
        function getKPIReports(statusStr) {
            $(".page-spinner-bar").removeClass("hide");
            var status = statusStr=='Month'?1:(statusStr=='Quater'?2:3)
            var reports = InsuranceAgentHomeService.getKPIReport(userId,status);
            reports.then(function (success) {
             ...
             $scope.quarterArr =['Jan-Mar','Apr-Jun','Jul-Sep','Oct-Dec'];
             $scope.quarter = $scope.quarterArr[getQuarter(new Date())-1];
        ...
        function DeletePolicy(deleteParam) {
            if(deleteParam.policyCanBeDeleted){
            bootbox.confirm({
                size: "",
                title: "Delete Policy",
        ...
        $scope.checkDisable = checkDisable;
        function checkDisable(item){
           return item.policyCanBeDeleted ? 
            {
                "cursor": "pointer",
                "color": "#df424f"
            } :
