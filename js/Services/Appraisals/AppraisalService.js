    ...
    this.getAppraisal = function (param) {
        var response = $http({
            method: "GET",
            url: AuthHeaderService.getApiURL() +"web/get/appraisal/"+param.appraisalId+"?agentId="+param.agentId+"&role="+param.role,
            headers: AuthHeaderService.getFileHeader()
        });
        return response;
    };
   ...
   // Update appraisal status - Active/In-Active 
    ...
    this.getRapnetDiamondPrice = function (param, url) {
        var response = $http({
            headers: {
