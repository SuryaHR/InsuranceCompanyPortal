angular.module('MetronicApp').service('HomeOwenerPolicyService', ['$http', '$rootScope', 'AuthHeaderService', function ($http, $rootScope, AuthHeaderService) {
    ...
    this.getPolicyTypeByState = function () {
        var response = $http({
            method: "GET",
            url: AuthHeaderService.getApiURL() + "web/policytypes/states",
            headers: AuthHeaderService.getHeader()
        });
        return response;
    }
    //DeletePolicyType
    this.DeletePolicyType = function (param) {
        var response = $http({
            method: "POST",
            data: param,
            url: AuthHeaderService.getApiURL() + "web/delete/policytypes",
            headers: AuthHeaderService.getHeader()
        });
        return response;
    }
